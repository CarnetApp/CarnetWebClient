    ...
    if (paths.length > 0) {
        RequestBuilder.sRequestBuilder.get("/metadata?" + paths, function (error, data) {
            for (var meta in data) {
                var note = new Note(Utils.cleanNoteName(FileUtils.getFilenameFromPath(meta)), data[meta].shorttext, meta, data[meta].metadata, data[meta].previews, false, data[meta].media)
...
function onListEnd(pathToList, files, metadatas, discret, force, fromCache) {
    ...
    if (!_.isEqual(files, oldFiles) || force) {
        ...
        for (let file of files) {
            var filename = FileUtils.getFilenameFromPath(file.path);
